...
export function ViewSubmissions() {
  ...
  getASMELSubmissions(userSession?.userId).then(response => setSubmissions(response.data));
    ...
    <Flex className="App-layout">
      ...
      >
        <Stack separator={<Flex height={"40px"} />} width={"50%"}>
          ...
          {submissions?.map(
              submission => {
                <Button onClick={() => navigate(submission?.documentUrl)}>{submission?.date}</Button>
              }
          )}
        </Stack>
      </Flex>
